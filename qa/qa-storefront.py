...
try:
    with open(os.path.expanduser("~/workspace/ecommerce-site/data/seed.json")) as _f:
        _seed = json.load(_f)
...
# ---------------------------------------------------------------- flows ---
def flows(pw):
    browser = pw.chromium.launch(executable_path="/opt/meta-chromium/chrome",
                                 headless=True, proxy=proxy_kwargs() or None)
    ...
    page.get_by_placeholder("Alex Morgan").fill("QA Tester")
    page.get_by_placeholder("you@example.com").fill(email)
    page.get_by_placeholder("At least 8 characters").fill("password123")
    page.get_by_placeholder("Repeat your password").fill("password123")
    page.locator('form button[type="submit"]').click()
    ...
    page.get_by_placeholder("you@example.com").fill("demo@novamart.com")
    page.get_by_placeholder("\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022").fill("wrongpassword")
    page.locator('form button[type="submit"]').click()
    page.wait_for_timeout(1200)
    alert = page.get_by_role("alert")
    alert_text = (alert.first.inner_text() if alert.count() else "").lower()
    check("2d. wrong password shows error", alert.count() > 0 and "sign you in" in alert_text,
    ...
    color_btns = page.get_by_role("button", name=re.compile(r"Select colour"))
    ...
    slugs = []
    for i in range(2):
        slugs.append(links.nth(i).get_attribute("href").split("/product/")[1])
    for s in slugs:
        goto(page, f"/product/{s}")
        cb = page.get_by_role("button", name=re.compile(r"Select colour"))
        if cb.count() > 0: cb.first.click(); page.wait_for_timeout(200)
        page.get_by_role("button", name="Add to cart").click(); page.wait_for_timeout(1000)
    ...
    check("6b. checkout step 2 -> 3 (Payment)",
          page.locator("#co-cardname").count() > 0 or "Payment" in page.locator("main").inner_text()[:2000])
    ...
    page.get_by_role("button", name=re.compile(r"Continue to review", re.I)).click()
    page.wait_for_timeout(1200)
    ...
    check("6d. order-success shows order number", bool(order_no) and order_no in page.locator("main").inner_text(),
          f"number={order_no}")
    n = cart_count(page)
    ...
    do_login(page, "demo@novamart.com", "demo123")
    goto(page, "/shop")
    s2 = page.locator('a[href^="/product/"]').nth(3).get_attribute("href").split("/product/")[1]
    goto(page, f"/product/{s2}")
    cb = page.get_by_role("button", name=re.compile(r"Select colour"))
    if cb.count() > 0: cb.first.click(); page.wait_for_timeout(200)
    page.get_by_role("button", name="Add to cart").click(); page.wait_for_timeout(1000)
    ...
    page.get_by_role("button", name=re.compile(r"Continue to review", re.I)).click(); page.wait_for_timeout(800)
    page.get_by_role("button", name=re.compile(r"Place order", re.I)).click()
    page.wait_for_url(re.compile(r"order-success"), timeout=15000)
    m = re.search(r"number=([^&]+)", page.url)
    ...
    check("7c. order appears in /account/orders",
          order2 in page.locator("main").inner_text(), f"looking for {order2}")

    # ---- 8. Order tracking -----------------------------------------------
    goto(page, "/track", "track")
    page.get_by_placeholder("e.g. NM-001001").fill(state["order1"])
    page.get_by_placeholder("you@example.com").fill("guest.qa@example.com")
    page.locator('form button[type="submit"]').click(); page.wait_for_timeout(1500)
    ...
    if add_w.count() == 0 and rem_w.count() == 0:
        ...
        wl_text = page.locator("main").inner_text()
    ...
    check("11. contact form success feedback",
          "Message received" in page.locator("main").inner_text())
    ...
    check("12b. gibberish search -> no-results state",
          re.search(r"no (products|results)|nothing found|0 products", page.locator("main").inner_text(), re.I) is not None,
          page.locator("main").inner_text()[:200].replace("\n", " "))
...
def screenshot_sweep(pw, vp_name, width, height, slug_hint):
    ...
    ctx = browser.new_context(viewport={"width": width, "height": height},
                              user_agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120 Safari/537.36" if width > 500
...
def main():
    ...
    with open(os.path.expanduser("~/workspace/ecommerce-site/qa/qa-storefront-results.json"), "w") as f:
